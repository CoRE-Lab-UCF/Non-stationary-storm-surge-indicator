...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             ...
             %%%Non stationary GEV model : Running window approach : Padding at the boundary
...
% EWL = seasonal maximum water level (e.g Summer) usually matrix of two column (year
% EWL). This could be a matrix with one column as well (EWL only)
...
[par_s CI_s]=gevfit(max(isnan(max)~=1)); 
...
%% Now take the range of the parameter values (Padding will have effect on the start and end portion (half of window length) of the time series   
...
for i=1:length(dat)
    for j=1:sim
	 ...
	 mu_ns(i,1)=nanmean(aa); % take the mean for the mean values
     mu_ns(i,2)=nanmin(bb); % take min for the lower uncertainty
     mu_ns(i,3)=nanmax(cc); % take the maximum for the upper uncertainty
...
for i=1:length(mu_ns) % iteration for total time steps of mu (for nonstationary);
    if isnan(mu_ns(i))~=1
        RWL{j}(i,1)=gevinv(1-1/RP(j),par_s(1),par_s(2),mu_ns(i,1)); % 1-1/RP(j) to estimate probability
        RWL{j}(i,2)=gevinv(1-1/RP(j),par_s(1),par_s(2),mu_ns(i,2)); % 2.5th percentile [only uncertainty in location parameter is considered]
        RWL{j}(i,3)=gevinv(1-1/RP(j),par_s(1),par_s(2),mu_ns(i,3)); % 97.5th percentile [only uncertainty in location parameter is considered]
    else
        RWL{j}(i,1)=NaN;
        RWL{j}(i,2)=NaN;
        RWL{j}(i,3)=NaN;
...
for i=1:length(mu_ns)
    if isnan(mu_ns(i))~=1
        ...
        RWL_s{j}(i,2)=gevinv(1-1/RP(j),par_s(1),par_s(2),CI_s(1,3)); % 2.5th percentile [only uncertainty in location parameter is considered]
        RWL_s{j}(i,3)=gevinv(1-1/RP(j),par_s(1),par_s(2),CI_s(2,3)); % 97.5th percentile [only uncertainty in location parameter is considered]
    else
        RWL_s{j}(i,1)=NaN;
        RWL_s{j}(i,2)=NaN;
        RWL_s{j}(i,3)=NaN;
